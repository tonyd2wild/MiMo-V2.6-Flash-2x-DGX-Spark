...
class TritonAttentionDiffKVBackend(TritonAttentionBackend):
    ...
    # fp8 (e4m3) KV: the reshape kernel quantizes on write; the attention kernel
    # upcasts K/V to the query dtype on load. KV scales are not applied on read,
    # so this is exact only for unit scales (the default; no calculated scales).
...
class TritonAttentionDiffKVImpl(TritonAttentionImpl):
    ...
    def forward(
        ...
        if is_quantized_kv_cache(self.kv_cache_dtype) and kv_cache.dtype != self.fp8_dtype:
