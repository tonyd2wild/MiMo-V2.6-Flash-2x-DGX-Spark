#!/usr/bin/env python3
"""Fixed-prompt benchmark for MiMo-V2.6-Flash-RL on vLLM (prompt set v1, byte-identical across boots).

Derived from the DeepSeek-V4.1-Flash bench. Adds a structured-extraction category and DFlash
acceptance (accepted tokens per draft step) read from the server /metrics around each batch.
...
usage: mimobench.py --base http://127.0.0.1:8888/v1 --model mimo-v2.6-flash --label boot4 --out DIR
...
WORDS_PER_TOKEN = 0.215  # filler words per MiMo token (measured: 0.55 words gave 2.54 tokens per target)
SUMMARY_PASSAGE = (
...
CATEGORIES = [
    ...
    ("structured", "Extract every person from this text into a YAML list with keys name, age (integer), city and job. Output only YAML.\n\nMaria Lopez, 34, is a nurse in Seville. Her brother Tomas (29) drives trucks out of Valencia. Their neighbour, the retired teacher Ingrid Olsen, is 71 and splits her time between Bergen and Seville, though her address is in Bergen. Ingrid's grandson Leo Olsen, 19, studies engineering in Oslo.", 200),
...
import re
SPEC_KEYS = ("vllm:spec_decode_num_drafts_total", "vllm:spec_decode_num_accepted_tokens_total")
def spec_counters(base):
    try:
        t = urllib.request.urlopen(base.rsplit("/v1", 1)[0] + "/metrics", timeout=10).read().decode()
    except Exception:
        return None
    return {k: sum(float(v) for v in re.findall("^" + re.escape(k) + r"(?:\{[^}]*\})? ([0-9.e+]+)$", t, re.M)) for k in SPEC_KEYS}
...
def run_batch(args, c, cat, prompt, max_tokens, tag):
    ...
    m0 = spec_counters(args.base)
    t0 = time.time()
    with ThreadPoolExecutor(max_workers=c) as ex:
        res = list(ex.map(one, range(c)))
    wall = time.time() - t0
    m1 = spec_counters(args.base)
    acc = None
    if m0 and m1 and m1[SPEC_KEYS[0]] > m0[SPEC_KEYS[0]]:
        acc = round((m1[SPEC_KEYS[1]] - m0[SPEC_KEYS[1]]) / (m1[SPEC_KEYS[0]] - m0[SPEC_KEYS[0]]), 2)
    ...
    return {"c": c, "category": cat, "wall_s": round(wall, 3), "tokens": toks,
            ...
            "ttft_mean_s": round(st.mean(ttfts), 3) if ttfts else None, "accept_per_draft": acc, "requests": res}
...
def main():
    ...
    ap.add_argument("--base", default="http://127.0.0.1:8888/v1"); ap.add_argument("--model", default="mimo-v2.6-flash")
    ...
    if args.dump_prompts:
        json.dump({"version": PROMPT_SET_VERSION, "temperature": 0, "thinking": False,
                   ...
                   "prefill_filler": {"words": FILLER_WORDS, "seed_rule": "seed = target tokens", "words_per_target_token": WORDS_PER_TOKEN}},
    ...
    for n, p, m in (CATEGORIES[:3] if levels else []):
        run_batch(args, 1, n, p, 64, "warm")
    for c in levels:
        for n, p, m in CATEGORIES + [CEILING]:
            b = run_batch(args, c, n, p, m, "run"); res["batches"].append(b)
            print(f"  C{c} {n:13} agg {b['agg_tok_s']:7.2f} tok/s  per-stream {b['per_stream_tok_s']}  ttft {b['ttft_mean_s']}s  acc/draft {b['accept_per_draft']}", flush=True)
    for tgt in pre:
        try:
            r = stream_chat(args.base, args.model, f"[bench {PROMPT_SET_VERSION} prefill {tgt}] " + filler(int(tgt * WORDS_PER_TOKEN), tgt)
                            + "\n\nReply with the single word OK.", 1, timeout=1800)
        except Exception as e:
            print(f"  prefill {tgt:>6}: FAILED {e}", flush=True); continue
    ...
    acc_rows = [[n] + [next((b["accept_per_draft"] for b in res["batches"] if b["c"] == c and b["category"] == n), None) for c in levels]
    ...
    md = [f"## {args.label} ({res['started']})", "", args.notes, "",
          ...
          "### DFlash accepted tokens per draft step (7 drafted per step)", "", md_table(["category"] + [f"C{c}" for c in levels], acc_rows), "",
